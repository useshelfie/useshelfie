...
export type ProductCatalogActionState = ActionState<Pick<Product, 'id' | 'company_id'> | undefined, { database?: string[] }>

// --- Helper Functions (Internal) ---

// Wrapper for schema validation
function validateCatalogForm(formData: FormData): { success: true; data: CatalogFormData } | { success: false; state: CatalogFormState } {
...
export async function createCatalogAction(
    ...
    try {
        ...
        return handleActionError(error, "Failed to create catalog.");
    }
}
...
export async function updateCatalogAction(
    ...
    try {
        ...
        return handleActionError(error, "Failed to update catalog.");
    }
}
...
export async function deleteCatalogAction(
    ...
    try {
        ...
        if (error.code === '23503' || (error.message && error.message.includes("violates foreign key constraint"))) {
            return handleActionError(error, "Cannot delete catalog: It still contains products.");
        }
        return handleActionError(error, "Failed to delete catalog.");
    }
}
...
export async function updateProductCatalogAction(
    ...
    try {
        ...
        if (newCatalogId === null) {
            ...
            updatedProductData = result as any; // Cast to handle potential mismatch from helper
        }
        ...
        if (typeof companyId !== 'number') {
            console.error("Critical: Could not determine a valid company ID for revalidation after product catalog update.", { initial: currentProduct?.company_id, fromUpdate: updatedProductData?.company_id });
        ...
        if (catalogIdsToFetch.length > 0) {
            ...
            if (slugError) {
                ...
                catalogData.forEach(c => { slugs[c.id] = c.slug; });
            }
        }
        ...
        const errorState = handleActionError(error, "Failed to update product catalog.") as ProductCatalogActionState;
