...
import { redirect } from "next/navigation" // Needed for revalidate + redirect pattern
...
function validateCategoryForm(formData: FormData): { success: true; data: { name: string } } |
{ success: false; state: CategoryFormState } {
...
export async function updateCategoryAction(
  ...
  // 2. Database Update
  try {
    // Fetch category first to check ownership (using RLS is better if configured)
    ...
    if (fetchError) throw new Error("Failed to fetch existing category data.");
    if (!existingCategory) throw new Error("Category not found.");
    if (existingCategory.user_id !== user.id || existingCategory.company_id !== companyId) {
      return handleActionError({}, "You do not have permission to update this category.");
    }
...
export async function deleteCategoryAction(
  ...
  if (isNaN(categoryId)) {
    return {
      message: "Invalid Category ID format. Expected a number.",
      type: "error",
      errors: { database: ["Invalid Category ID format. Expected a number."] }
    }
  }
