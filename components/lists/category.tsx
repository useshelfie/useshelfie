...
import { deleteCategoryAction, updateCategoryAction, type CategoryFormState } from "@/lib/actions/category"
...
function CategoryItem({ category }: { category: Category }) {
  ...
  const handleUpdate = async (formData: FormData) => {
    const result = await updateCategoryWithId(initialState, formData)
    if (result.type === "success") {
  ...
  return (
    <div className="flex items-center justify-between p-4 border rounded-md hover:bg-muted/50 transition-colors">
      {isEditing ? (
        <form action={handleUpdate} className="flex-grow flex items-center gap-2 mr-2">
          <Input
            name="name"
        ...
        <span className="font-medium flex-grow truncate mr-2" title={category.name}>
          {category.name}
        </span>
      )}
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Deleting the category &quot;<span className="font-semibold">{category.name}</span>&quot; cannot be
                  undone. Products using this category might be affected.
                </AlertDialogDescription>
              </AlertDialogHeader>
              {deleteError && (
